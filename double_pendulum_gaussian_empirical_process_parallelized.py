...
def _path_long_run_covariance(x: np.ndarray, grid: np.ndarray, Fhat: np.ndarray, L: int) -> np.ndarray:
...
def estimate_long_run_covariance(
    ...
    def compute_and_report(x: np.ndarray) -> np.ndarray:
        nonlocal completed
        sigma_i = _path_long_run_covariance(x, grid, Fhat, L)
        with lock:
            completed += 1
    ...
    jobs = [
        {
            ...
            "out_dir": os.path.join("gaussian_empirical_process_output2", "paths1"),
        },
        {
            "paths_folder": "paths2",
            "results_csv": os.path.join("AVMs2", "paths2_results.csv"),
            "out_dir": os.path.join("gaussian_empirical_process_output2", "paths2"),
        }
    ]
